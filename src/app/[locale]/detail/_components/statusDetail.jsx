...
const Status = ({ status }) => {
  ...
  return (
    <div className="w-100 mt-4 h-[42px] px-[33.5px]">
      <div
        className={`text-center bg-transparent ${text_colors[status]} border-opacity-100 border-solid noto-sans-jp-700 lg:text-[14px] md:text-[14px] text-[12px] font-semibold py-2 px-4 border-2 ${border_colors[status]} rounded-lg w-[100%]`}
      >
        {message[status]}
      </div>
    </div>
